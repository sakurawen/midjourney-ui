...
export class MidjourneyMessage {
  ...
  async FilterMessages(prompt: string, loading?: LoadingHandler, options?: string, index?: number) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (item.author.id === '936929561302675456' && item.content.includes(`**${prompt}`)) {
        ...
        if (
          options &&
          !(item.content.includes(options) || (options === 'Upscaled' && item.content.includes(`Image #${index}`)))
        ) {
          this.log('no options');
          continue;
        }
  ...
  async WaitOptionMessage(content: string, options: string, loading?: LoadingHandler) {
    for (let i = 0; i < this.maxWait; i++) {
  ...
  async WaitUpscaledMessage(content: string, index: number, loading?: LoadingHandler) {
    for (let i = 0; i < this.maxWait; i++) {
      const msg = await this.FilterMessages(content, loading, 'Upscaled', index);
      if (msg !== null) {
        return msg;
      }
      this.log(i, content, 'wait no message found');
  ...
  async RetrieveMessages(limit = 50) {
    ...
    const response = await request(`https://discord.com/api/v10/channels/${this.ChannelId}/messages?limit=${limit}`, {
      headers: headers,
    });
